...
3. Geminiのリクエスト数も表示する場合は、同じサービスアカウントにプロジェクトレベルの
   **Monitoring Viewer** を付与し、Cloud Monitoring APIを有効化する
4. JSON鍵を発行し、その中身を `GOOGLE_SERVICE_ACCOUNT_JSON` に**1行のJSON文字列として**設定する
   (ファイルパスではなく中身そのもの)
...
BigQueryの請求エクスポートにはリクエスト数が含まれないため、アプリはCloud Monitoringの
`serviceruntime.googleapis.com/api/request_count` を日次集計して表示する。Gemini API
(`generativelanguage.googleapis.com`) とVertex AI (`aiplatform.googleapis.com`) が対象となる。
Vertex AI APIをGemini以外にも利用しているプロジェクトでは、そのリクエストも合算されるため、
Gemini専用プロジェクトでの利用を推奨する。

Monitoring Viewer権限またはCloud Monitoring APIが未設定の場合も、料金・トークンの同期は継続し、
リクエスト数だけが「取得不可」と表示される。
